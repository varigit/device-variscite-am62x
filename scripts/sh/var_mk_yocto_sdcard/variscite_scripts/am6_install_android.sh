...
# Detect SoC family + board (drives image filenames below).
soc_family=$(cat /sys/devices/soc0/family 2>/dev/null)
dt_model=$(tr -d '\0' < /proc/device-tree/model 2>/dev/null)

if [[ "${soc_family}" == *AM62PX* ]] || [[ "${soc_family}" == *AM62P* ]]; then
	node=/dev/mmcblk0
	som="am62p"
	case "${dt_model}" in
		*VAR-SOM-AM62P*Symphony*) board="am62p-var-som-symphony" ;;
		*) red_bold_echo "ERROR: Unsupported AM62P carrier: '${dt_model}'"; exit 1 ;;
	esac
...
usage()
{
	...
	echo " -u                install unsigned dtbo generated building android with AVB feature disabled"
	echo
}
...
function check_images
{
	...
	if [[ ! -f ${imagesdir}/${persistimage_file} ]] ; then
		red_bold_echo "ERROR: ${persistimage_file} image does not exist"
		exit 1
	fi
...
function install_android
{
	...
	blue_underlined_bold_echo "Installing persist partition"
	dd if=${imagesdir}/${persistimage_file} of=${node}${part}21 bs=1M conv=fsync
